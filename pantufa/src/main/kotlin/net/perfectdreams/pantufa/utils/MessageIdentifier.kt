...
data class BaseMessagePanelData(
    ...
    fun <T> fetchWithCurrencyType(page: Long, currency: TransactionCurrency) = transaction(Databases.sparklyPower) {
        (data as SizedIterable<T>)
            .limit(type.entriesPerPage, page * type.entriesPerPage)
            .filter { it is TTransaction && it.currency == currency }
            .toList()
    }
}
