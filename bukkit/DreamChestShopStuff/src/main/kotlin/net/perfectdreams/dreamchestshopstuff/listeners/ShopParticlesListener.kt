...
class ShopParticlesListener(val m: DreamChestShopStuff) : Listener {
    ...
    fun onTransaction(event: PreTransactionEvent) {
        if (event.transactionOutcome == PreTransactionEvent.TransactionOutcome.TRANSACTION_SUCCESFUL) {
            event.client.world.spawnParticle(Particle.VILLAGER_HAPPY, event.client.location.add(0.0, 0.5, 0.0), 3, 0.5, 0.5, 0.5)
        } else {
            event.client.world.spawnParticle(Particle.VILLAGER_ANGRY, event.client.location.add(0.0, 0.5, 0.0), 3, 0.5, 0.5, 0.5)
        }
    }
}