...
class SparklyCommandManager(val plugin: Plugin) {
    ...
    fun register(command: SparklyCommandDeclarationWrapper, vararg executors: SparklyCommandExecutor) {
        ...
        for (brigadierCommand in commands) {
            commodore.register(brigadierCommand as LiteralArgumentBuilder<CommandListenerWrapper>) // TODO: Remove this cast
        }
    }

    // TODO: How to unregister brigadier commands?

    private fun convertDeclarationToBrigadier(declaration: SparklyCommandDeclaration): List<ArgumentBuilder<CommandListenerWrapper, *>> {
        val commands = mutableListOf<ArgumentBuilder<CommandListenerWrapper, *>>()

        val executor = Command<CommandListenerWrapper> { commandContext ->
        ...
        for (label in declaration.labels) {
            commands += LiteralArgumentBuilder.literal<CommandListenerWrapper>(label).apply {
                ...
                if (declaration.executor != null) {
                    if (declaration.executor.options.arguments.isEmpty()) {
                        ...
                        val arguments = declaration.executor.options.arguments.map {
                            when (it) {
                                // ===[ STRING ]===
                                is GreedyStringCommandOption, is OptionalGreedyStringCommandOption -> RequiredArgumentBuilder.argument<CommandListenerWrapper, String>(
                                    it.name,
                                    StringArgumentType.greedyString()
                                )
                                is QuotableStringCommandOption -> RequiredArgumentBuilder.argument<CommandListenerWrapper, String>(
                                    it.name,
                                    StringArgumentType.string()
                                )
                                is WordCommandOption -> RequiredArgumentBuilder.argument<CommandListenerWrapper, String>(
                                    it.name,
                                    StringArgumentType.word()
                                )

                                // ===[ BOOLEAN ]===
                                is BooleanCommandOption -> RequiredArgumentBuilder.argument<CommandListenerWrapper, Boolean>(
                                    it.name,
                                    BoolArgumentType.bool()
                                )

                                // ===[ DOUBLE ]===
                                is DoubleCommandOption -> RequiredArgumentBuilder.argument<CommandListenerWrapper, Double>(
                                    it.name,
                                    DoubleArgumentType.doubleArg()
                                )
                                is DoubleMinCommandOption -> RequiredArgumentBuilder.argument<CommandListenerWrapper, Double>(
                                    it.name,
                                    DoubleArgumentType.doubleArg(it.min)
                                )
                                is DoubleMinMaxCommandOption -> RequiredArgumentBuilder.argument<CommandListenerWrapper, Double>(
                                    it.name,
                                    DoubleArgumentType.doubleArg(it.min, it.max)
                                )

                                // ===[ PLAYER ]===
                                is PlayerCommandOption -> RequiredArgumentBuilder.argument<CommandListenerWrapper, EntitySelector>(
                                    it.name,
                                    ArgumentEntity.c()
                        ...
                        val nodes = mutableListOf<ArgumentBuilder<CommandListenerWrapper, *>>(this).apply {
