...
class Hologram(val location: Location, var text: String) {
	...
	private val dataWatcher = WrappedDataWatcher().apply {
		...
		setObject(name, getWrappedName(text)) // Name
	...
	private val attach = WrapperPlayServerAttachEntity().apply {
		entityID = ID
	}
	...
	fun updateText(text: String, player: Player) {
		this.text = text
		dataWatcher.setObject(name, getWrappedName(text))
	...
	private fun getWrappedName(name: String) = Optional.of(WrappedChatComponent.fromChatMessage(name)[0].handle)
}

class WrapperHologram(val location: Location, lines: MutableList<String>) {
	private val viewers = Collections.newSetFromMap(WeakHashMap<Player, Boolean>())
